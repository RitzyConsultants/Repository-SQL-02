select ST.*,GroupByField = '',GroupByFieldCode = '',GroupByFieldDesc = '',SecondGroupByField = '',SecondGroupByFieldCode = '',
	   ...
	   where (AccountLink=S.StockLink) and (IsNull(WarehouseID,0)=IsNull(ST.iWarehouseID,0))  and TxDate>'2015-03-31'), 0)
...
group by iStockID, iWarehouseID ) CT on ST.StockLink = CT.LastStockID and ST.iWarehouseID = CT.iWarehouseID  and ST.dTxDate = CT.LastTxDate Left outer join StkItem S on S.StockLink = ST.StockLink
where ((((ST.Whseitem = 0) or ((ST.Whseitem = 1) and (ST.iWarehouseID > 0)))) and (ST.Code>='Gary') and (ST.Code<='Gary') and (ST.ItemActive=1)) and (ST.ServiceItem<>1)
order by ST.Code
...
select ST.*,GroupByField = '',GroupByFieldCode = '',GroupByFieldDesc = '',SecondGroupByField = '',SecondGroupByFieldCode = '',
	   ...
	   + IsNull( (select case when ST.ServiceItem = 0 then IsNull(sum (QtyOut),0) else 0 end from _bvSTTransactionsFull 
	   where (AccountLink=S.StockLink) and (IsNull(WarehouseID,0)=IsNull(ST.iWarehouseID,0))  and TxDate>'2015-03-31') 
	   + (select case when ST.ServiceItem = 0 then IsNull(sum (fJCWIPQuantity),0) else 0 end from _bvSTTransactionsFull 
	   where (AccountLink=S.StockLink) and (IsNull(WarehouseID,0)=IsNull(ST.iWarehouseID,0))  and TxDate>'2015-03-31') 
	   - (select case when ST.ServiceItem = 0 then IsNull(sum (QtyIn), 0) else 0 end from _bvSTTransactionsFull 
	   where (AccountLink=S.StockLink) and (IsNull(WarehouseID,0)=IsNull(ST.iWarehouseID,0))  and TxDate>'2015-03-31'), 0)
from _evInvCostTracking ST join (select iStockID as LastStockID, iWarehouseID,  MAX(dTxDate) as LastTxDate
