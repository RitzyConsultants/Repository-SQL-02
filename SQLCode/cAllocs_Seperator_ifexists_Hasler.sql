...
WHERE id = object_id(N'[dbo].[cAllocs_Seperator]')
and OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].cAllocs_Seperator
GO

CREATE PROC [dbo].[cAllocs_Seperator]	@FROM_DATE VARCHAR(MAX),
								@TO_DATE VARCHAR(MAX)

AS

--DROP THE TABLE IF EXISTS
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Temp_Allocation]') AND type in (N'U'))
DROP TABLE [dbo].[Temp_Allocation] 
